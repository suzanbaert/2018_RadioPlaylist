...
Their second album *No Need to Argue* was one of my favourite CD's as a teenager. So I wondered: how many times would they be played? And it would it all be 'Zombie', or would some of their other gems get air time as well?  
<br>
...
I wrote a `read_playlist` funcion to scrape the data into a dataframe with 5 columns: station, date, timestamp, artist and title. I immediately made a second version for the instances where I would need to iterate this function, called `read_playlist_and_sleep` which just adds 5 seconds in between every iteration.
```{r}
...
So curious... 
But before doing breakouts I wanted to make sure that no-one had written it differently, so I so did a str_view just to check that I wouldn't miss any instance in my next codes.   
...
Not entirely what I would have expected: the rock-indie-pop station leads the list, followed by the current affairs Radio1, and the oldies-channel Nostalgie. The hit radio stations follow the list.
```{r}
playlist_TheCranberries %>%
  filter(artist == "The Cranberries") %>%
  group_by(radio) %>%
...
No surprise: Zombie leads the list, but what is left of the teenager inside of me is a bit sad that so many of their other songs are not in there. I would have played the supersad: No need to argue - it would have fitted.
```{r}
playlist_TheCranberries %>%
  filter(artist == "The Cranberries") %>%
  group_by(title) %>%
...
The biggest issue is that I have no frame of reference: is 36 times a lot, or not? France Gall passed away not too long ago, and Tom Petty. I did a bit of search for artists who passed away recently, and some slightly longer ago so I had some big names to compare as well.  
...
Additionally there were some oddities driven by timezones, so I decided to look up in one of our biggest newspapers when the news broke about every artist and take that date as a starting point.
...
I ended up with 9 databases of more than 17000 rows, most of which I did not need. The next bit of code finds the lines where the artist who just died (or will die a week later) is played, and then merges it all together.  
